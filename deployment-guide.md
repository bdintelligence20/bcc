...
- **Maven Parent POM Resolution**: The Java backend services (ruoyi-admin and ruoyi-web) were failing to build because Maven couldn't resolve the parent POM. The error was: "Non-resolvable parent POM for com.ruoyi:ruoyi-admin:3.4.0: The following artifacts could not be resolved: com.ruoyi:ruoyi-beiqi:pom:3.4.0 (absent)". This was fixed by running the Maven build from the root directory of the project, where the parent POM is located, instead of from the module directory. The cloudbuild.yaml file has been updated to build the entire project at once, which ensures that all modules are built in the correct order and with the correct parent POM.
