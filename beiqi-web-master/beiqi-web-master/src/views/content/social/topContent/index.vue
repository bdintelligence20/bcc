  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="状态" prop="status">
        ...
        /> -->
        <el-select v-model="queryParams.status" placeholder="请输入状态" size="small" clearable>
    ...
    <el-dialog :title="title" :visible.sync="open" fullscreen append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-position="top" label-width="150px">
        <el-row :gutter="40">
          <el-col :span="8">
            <el-form-item label="社会责任内容（英语）" prop="contentEn">
              <editor ref="editor" v-model="form.contentEn" :min-height="192" :height="360" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="社会责任内容（西班牙语）" prop="contentEs">
              <editor ref="editor" v-model="form.contentEs" :min-height="192" :height="360" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="社会责任内容（阿拉伯语）" prop="contentAr">
              <editor ref="editor" v-model="form.contentAr" :min-height="192" :height="360" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="状态" prop="status">
...
<script>
import { listContent, getContent, delContent, addContent, updateContent } from "@/api/social/content";
...
export default {
  ...
  methods: {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateContent(formArray).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addContent(formArray).then(response => {
    ...
    resolveFormData() {
      ...
      var reqArray = []
      let { langData, contentEs, contentEn, contentAr, ...newObj } = this.form
      if(langData.length < 1) {
        languageList.forEach(i => {
          let obj = JSON.parse(JSON.stringify(newObj))
          obj.language = i
          let lang = titleCase(i)
          obj.content = this.form['content' + lang]
          reqArray.push(obj)
        })
      } else {
        langData.forEach( (data, i) => {
          let lang = titleCase(data.language)
          langData[i].content = this.form['content' + lang]
    ...
    handleDelete(row) {
      const ids = row.id || this.ids;
      this.$modal.confirm('是否确认删除社会责任内容编号为"' + ids + '"的数据项?', "警告", {
