<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="jobList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="招聘岗位名称" align="center" prop="name" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-position="top" label-width="150px">
        <el-form-item label="招聘岗位名称（英语）" prop="nameEn">
          <el-input v-model="form.nameEn" placeholder="请输入英文名称" />
        </el-form-item>
        <el-form-item label="招聘岗位名称（西班牙）" prop="nameEs">
          <el-input v-model="form.nameEs" placeholder="请输入西班牙名称" />
        </el-form-item>                   
        <el-form-item label="招聘岗位名称（阿拉伯）" prop="nameAr">
...
<script>
import { listJobCategory, getJobCategory, delJobCategory, addJobCategory, updateJobCategory } from "@/api/joinUs/jobCategory";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getJobCategory(ids).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateJobCategory(formArray).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addJobCategory(formArray).then(response => {
    ...
    handleDelete(row) {
      const ids = row.id || this.ids;
      this.$modal.confirm('是否确认删除招聘岗位编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delJobCategory(ids);
        }).then(() => {
          this.getList();
