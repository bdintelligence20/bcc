<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="areaList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="招聘区域名称" align="center" prop="name" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-position="top" label-width="150px">
          <el-form-item label="招聘区域名称（英语）" prop="nameEn">
            <el-input v-model="form.nameEn" placeholder="请输入英文名称" />
          </el-form-item>
          <el-form-item label="招聘区域名称（西班牙）" prop="nameEs">
            <el-input v-model="form.nameEs" placeholder="请输入西班牙名称" />
          </el-form-item>                   
          <el-form-item label="招聘区域名称（阿拉伯）" prop="nameAr">
...
<script>
import { listArea, getArea, delArea, addArea, updateArea } from "@/api/joinUs/area";
...
export default {
  ...
  methods: {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateArea(formArray).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addArea(formArray).then(response => {
    ...
    handleDelete(row) {
      const ids = row.id || this.ids;
      this.$modal.confirm('是否确认删除招聘区域编号为"' + ids + '"的数据项?', "警告", {
