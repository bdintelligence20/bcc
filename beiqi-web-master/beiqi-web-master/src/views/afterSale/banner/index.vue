  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" fullscreen append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-position="top" label-width="150px">
        <el-row :gutter="40">
          <el-col :span="7">
            <el-form-item label="Banner内容（英语）" prop="contentEn">
              <editor ref="editor" v-model="form.contentEn" :min-height="192" :height="360" />
            </el-form-item>
          </el-col>
          <el-col :span="7">
            <el-form-item label="Banner内容（西班牙语）" prop="contentEs">
              <editor ref="editor" v-model="form.contentEs" :min-height="192" :height="360" />
            </el-form-item>
          </el-col>
          <el-col :span="7">
            <el-form-item label="Banner内容（阿拉伯语）" prop="contentAr">
...
<script>
import { listBanner, getBanner, delBanner, addBanner, updateBanner } from "@/api/afterSale/banner";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加售后页banner";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const bannerId = row.id || this.ids;
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除售后banner编号为"' + bannerIds + '"的数据项?', "警告", {
