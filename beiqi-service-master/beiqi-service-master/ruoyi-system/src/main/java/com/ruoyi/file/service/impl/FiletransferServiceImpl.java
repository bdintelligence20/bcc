...
public class FiletransferServiceImpl implements IFiletransferService {
    ...
    @Override
    synchronized public void uploadFile(HttpServletRequest request, UploadFileDTO uploadFileDto, Long userId) {
        ...
        for (UploadFile file : uploadFileList) {
            ...
            if (uploadFile.getSuccess() == 1) {
                ...
                fileBeanLambdaQueryWrapper.eq(FileBean::getIdentifier, uploadFileDto.getIdentifier());
                FileBean fileBeanDb = fileMapper.selectOne(fileBeanLambdaQueryWrapper);
                if (fileBeanDb != null) {
                    return;
                }
    ...
    @Override
    public AjaxResult downloadImage(String imagePath, String batch) {
        AjaxResult ajax = AjaxResult.success();
        System.out.println("threadPoolTaskExecutor 当前线程:" + Thread.currentThread().getName() );
        HashMap<String, Object> hashMap = new HashMap<>();
        if (batch == null || batch.length() == 0 || batch.isEmpty()){
            String baseDir = RuoYiConfig.getImagePath();
            String imageUrl = null;
            try {
                imageUrl = FileUploadUtils.downloadImage(baseDir, imagePath);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            hashMap.put("url", imageUrl);
            hashMap.put("fileName", imageUrl);
            hashMap.put("newFileName", FileUtils.getName(imageUrl));
            ajax.put("map",hashMap);
            return ajax;
        } else {
            String key = Constants.DOWNLOAD_IMAGE + imagePath + "-" + batch;
            HashMap<String, Object> link = redisCache.getCacheObject(key);
            if (link == null) {
                String baseDir = RuoYiConfig.getImagePath();
                String imageUrl = null;
                try {
                ...
                hashMap.put("fileName", imageUrl);
                hashMap.put("newFileName", FileUtils.getName(imageUrl));
                redisCache.setCacheObject(key,hashMap,48, TimeUnit.HOURS);
                ajax.put("map",hashMap);
                return ajax;
            }else {
                return ajax.put("map",link);
            }
        }
    }
